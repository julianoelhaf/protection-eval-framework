"""tab:fl_observability -- MAE (% line) under full / relay-pair / single-relay sensing.

Full = the 20/50 ms reference (repro_fl), reported as mean $\\pm$ standard deviation over
...
Column-best (min mean) is bolded. All committed.
"""
import sys
...
def build():
    ...
    full = {c: repro[("y_fault_location", FL_KEY[c[0]], "W=0.0%s" % c[1])] for c in COLS}
    pair = {c: S.obs_group_stats(obs, "fl", c[0], c[1], "p", 4) for c in COLS}
    single = {c: S.obs_group_stats(obs, "fl", c[0], c[1], "s", 8) for c in COLS}

    means = {"Full": {c: full[c]["mean"] for c in COLS},
             "Relay pair": {c: pair[c][0] for c in COLS},
             "Single relay": {c: single[c][0] for c in COLS}}
    best = {c: min(means, key=lambda r: means[r][c]) for c in COLS}  # min MAE
...
\end{table*}""" % "\n".join(body)
    ...
    S.write_table("fl_observability", latex,
                  "run_obs aggregate (mean + min--max range) + repro_fl full (mean +/- std) (committed)")
