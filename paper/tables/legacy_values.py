"""Cells with NO committed regeneration in ``reports/`` (as of 2026-08-06).

These are transcribed verbatim from the manuscript -- they are the disclosed
original-run / W&B values covered by its reproducibility note, and only the
30/40 ms timing cells remain. Any table generator that reads from here also emits a WARNING via
``sources.warn`` so the provenance report flags exactly which cells are not
committed-backed.
