#!/bin/bash -l
#SBATCH --job-name=ml_regression__runtime_analysis
#SBATCH --ntasks=1
...
PROJECT_DIR="/path/to/repos/${REPOSITORY_NAME}"
...
# export https_proxy=http://<your-site-proxy>:<port>   # only if your cluster needs a proxy for W&B
...
for W in "${WINDOW_LENGTHS[@]}"; do
  for MODEL_NAME in "${REGRESSOR_MODELS[@]}"; do
    for step_length_seconds in "${STEP_LENGTHS[@]}"; do
...
echo "All regression jobs completed at $(date)"
