...
def write_filtered_memmap(
    ...
    # RACE-SAFE: never write to a fixed shared path. Concurrent jobs (e.g. SLURM-array folds of the
    # same config) previously opened the SAME out_path with mode="w+", so one job could truncate and
    # rewrite the file while another was still reading it mid-computation -> corrupted rows. Write to
    # a PROCESS-UNIQUE path so two jobs can never collide, and schedule removal at process exit.
    # Callers use only the returned array, so the internal filename is irrelevant.
    unique_path = out_path.with_name(
        f"{out_path.stem}.pid{os.getpid()}.{uuid.uuid4().hex}{out_path.suffix}"
    )
    atexit.register(lambda p=unique_path: p.unlink(missing_ok=True))

    X_out = np.memmap(unique_path, mode="w+", dtype=dtype, shape=(n_out, L, F))
...
def main(config: MainConfig) -> None:
    ...
    for i, (train_idx, test_idx) in enumerate(
        ...
        logger.info(f"Split {i} training time: {time.time()-model_time:.2f} seconds")
    ...
    keep_cols = [c for c in oof_label_cols if c in oof_df.columns] + [
        "fold",
        "y_true",
        "y_pred",
    ]
