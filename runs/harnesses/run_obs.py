"""Observability regeneration harness (pinned-pipeline).

Mirrors run_fc.py but inserts select_relay_features() so the reduced-
observability rows (single_relay / relay_subset) are regenerated under the
EXACT pinned pipeline: per-fold train-only StandardScaler, deterministic
GroupKFold, same fault-only alignment. FC -> macro-F1, FL -> MAE.

Emits a full per-run lifecycle log to stdout (the committed SLURM .out):
RUN/ENV -> LOAD -> RELAY (feature reduction) -> DATA -> per-fold FOLD -> the
parsed metric line -> DONE. Only the metric line is appended to RUN_OUT.

Usage:
    python run_obs.py <tag> <hydra_override> [<hydra_override> ...]
Env:
    WINDIR     -> window_extraction.windows_local_dir
...
RESULTS = os.environ.get("RUN_OUT", "/path/to/scratch/run_obs_results.txt")
...
def main():
    ...
    from fcl_psp.models.run_model import get_sample_ids_and_fault_targets, select_relay_features
    ...
    # Reduce on the FEATURE axis FIRST (memmap -> small in-memory array), then subset the
    # FAULT-ONLY rows in memory. Orthogonal axes: (X[:,:,cols])[ri] == (X[ri])[:,:,cols].
    f_full = int(windows.shape[2])
    windows, ablation_meta = select_relay_features(windows, cfg)
    windows = np.asarray(windows)
    p(tag, "RELAY", mode=cfg.ablation.mode, feat_before=f_full, feat_after=int(windows.shape[2]))
    if ri is not None:  # FL: loader already filtered `labels` to these rows
        windows = windows[ri]
    ...
    arr = np.array(scores)
    log(
        f"[{tag}] {name} mean={arr.mean():.4f} std={arr.std():.4f} "
        f"nfeat={F} per_fold={[round(x, 4) for x in scores]} "
        f"overrides={extra} ({time.time() - t0:.0f}s)"
    )
