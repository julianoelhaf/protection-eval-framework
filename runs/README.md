...
per-fold train-only `StandardScaler`. They are committed as **provenance**: every regenerated number can be
traced from the metric-computing line in these files to the output file under `reports/runs/` that the value
was read from.
...
## Run logs (`logs/`)
The raw SLURM per-job console logs are retained in the authors' internal repository — the audit chain is only
complete if the run itself is inspectable, not just its parsed result. Each log records the run's tag, the full
Hydra override string, timestamps, the execution node, and any traceback. **They are not part of this public
release** (they carry site-specific hostnames and absolute scratch paths); the aggregated results they produced
are the `reports/` files published here. The campaign map below documents that chain.
...
**Policy:** raw per-run logs are recorded as each run finishes; parsed/aggregated result files are committed
